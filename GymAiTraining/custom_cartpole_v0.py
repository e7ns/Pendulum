...
class CustomCartPoleEnv(gym.Env[np.ndarray, Union[int, np.ndarray]]):
    ...
    * v0: Initial versions release.
    ...
    def step(self, action):
        ...
        if not terminated:
            reward = 0.0 if self._sutton_barto_reward else 10.0 -  2 * np.abs(x / self.x_threshold) - 4 * np.abs(theta / self.theta_threshold_radians)
    ...
    def normalizeStateAngles(self):
        ...
        x, x_dot, theta, theta_dot = self.state
        if theta_dot <= -math.pi: theta_dot += 2 * math.pi
        if theta_dot > math.pi: theta_dot -= 2 * math.pi
