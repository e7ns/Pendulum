...
class CustomCartPoleEnv(gym.Env[np.ndarray, Union[int, np.ndarray]]):
    ...
    * v0: Initial versions release.
    ...
    def __init__(
        ...
        self.length = 0.15  # actually half the pole's length
    ...
    def step(self, action):
        ...
        actionToForce = (-self.force_mag, -0.5 * self.force_mag, 0, 0.5* self.force_mag, self.force_mag)
        force = actionToForce[action]
        ...
        effectiveness = 0.98
        theta_dot *= effectiveness
        x_dot *= effectiveness

        #print(f"Rot Accel {thetaacc}")
        #print(f"Rot V {theta_dot}")
        #print(f"Rot {theta}")
        ...
        if not terminated:
            reward = 0.0 if self._sutton_barto_reward else 20.0 -  6 * np.abs(x / self.x_threshold) - 5 * np.abs(theta / self.theta_threshold_radians)
    ...
    def reset(
        ...
        *,
        ...
        # state/observations.
        x = np.random.uniform(-0.3 * self.x_threshold, 0.3  * self.x_threshold )
    ...
    def render(self):
        ...
        text_surface = self.my_font.render('Use Arrow Keys to Force Movement', False, (0, 0, 0))
